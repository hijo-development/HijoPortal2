        ...
        public static DataTable Preview_CA(string DOC_NUMBER, string entitycode)
        {
            ...
            if (TRAIN)
            {
                for (int c = 0; c < farm_list.Count; c++)
                {
                    ...
                    bool execute = entitycode == Constants.TRAIN_CODE();
                    if (execute)
                        cmd = new SqlCommand(query_1);
                    else
                        cmd = new SqlCommand(query_2);
