        ...
        public static DataTable MRP_OPEX(string DOC_NUMBER, string entitycode)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    dtRow["isallowedProcCat"] = row["isallowedProcCat"].ToString();
                    dtRow["ItemCode"] = row["ItemCode"].ToString();
